...
QString PacketCapture::createHexDump(const u_char* data, int len)
{
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (data[i] >= 32 && data[i] <= 126) {
            asciiDump += QChar(data[i]);
        } else {
            asciiDump += '.';
        }
    }

    int remaining = len % bytesPerLine;
    if (remaining > 0) {
        int padding = (bytesPerLine - remaining) * 3;
        hexDump += QString("%1").arg("", padding, ' ');
    }

    hexDump += QString("  |  %1").arg(asciiDump);
    return hexDump;
}
    ...
    switch (ip_header->ip_p) {
        ...
        case IPPROTO_ICMP: {
            ...
            switch(icmp_header->type) {
                ...
                case ICMP_SOURCE_QUENCH: icmpType = "Source Quench"; break;
                case ICMP_REDIRECT: icmpType = "Redirect"; break;
                case ICMP_ECHO: icmpType = "Echo Request"; break;
                case ICMP_TIME_EXCEEDED: icmpType = "Time Exceeded"; break;
                case ICMP_PARAMETERPROB: icmpType = "Parameter Problem"; break;
                case ICMP_TIMESTAMP: icmpType = "Timestamp Request"; break;
                case ICMP_TIMESTAMPREPLY: icmpType = "Timestamp Reply"; break;
                case ICMP_INFO_REQUEST: icmpType = "Information Request"; break;
                case ICMP_INFO_REPLY: icmpType = "Information Reply"; break;
                default: icmpType = QString::number(icmp_header->type); break;
            }
